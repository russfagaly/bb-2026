...
<style>
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  .modal-hdr {{ padding:14px 16px 12px; color:#fff; }}
  .modal-body {{ padding:12px 14px 24px; }}
  .modal-close {{ float:right; font-size:22px; opacity:.7; background:none; border:none; color:#fff; cursor:pointer; line-height:1; }}

  /* ── STAT PILLS ── */
  .pill {{ background:#f8fafc; border-radius:10px; padding:8px 6px; text-align:center; }}
...
function renderPlayerCards(data) {{
  ...
  data.forEach(p => {{
    ...
    card.innerHTML=`
      <div style="background:${{c.bg}};padding:12px 8px;text-align:center">
        <div style="width:40px;height:40px;border-radius:50%;background:${{c.accent}};margin:0 auto 6px;display:flex;align-items:center;justify-content:center;font-weight:900;color:${{c.bg}};font-size:15px">${{p.jersey||'?'}}</div>
        <div style="color:#fff;font-weight:700;font-size:13px;line-height:1.2">${{p.name}}</div>
        <div style="color:rgba(255,255,255,.65);font-size:11px;margin-top:2px">${{roles.join(' ')}}</div>
      </div>
      <div style="padding:8px 10px;text-align:center;background:#fff">
        ${{p.hitting?`<div style="color:#1e3a5f;font-weight:700;font-size:12px">AVG ${{p.hitting.avg}}</div>`:''}}
        ${{p.pitching?`<div style="color:#1a4731;font-weight:700;font-size:12px">ERA ${{p.pitching.era}}</div>`:''}}
