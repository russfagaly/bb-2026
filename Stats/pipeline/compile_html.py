...
<style>
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  .modal-box {{ background:#fff; border-radius:20px 20px 0 0; width:100%; max-width:640px;
                max-height:92vh; overflow-y:auto; box-shadow:0 -4px 30px rgba(0,0,0,.2);
                animation:slideUp .22s ease; }}
  @keyframes slideUp {{ from {{ transform:translateY(60px); opacity:0; }} to {{ transform:translateY(0); opacity:1; }} }}
  .modal-drag {{ width:36px; height:4px; background:#e2e8f0; border-radius:2px; margin:10px auto 0; }}
  .modal-hdr {{ position:relative; padding:14px 50px 14px 16px; color:#fff; }}
  .modal-body {{ padding:12px 14px 24px; }}
  ...
  /* ── STAT PILLS ── */
  .pill {{ background:#f8fafc; border-radius:10px; padding:9px 6px; text-align:center; }}
  .pill-val {{ font-size:15px; font-weight:800; color:#0f2744; line-height:1; }}
  .pill-lbl {{ font-size:9px; color:#94a3b8; font-weight:600; text-transform:uppercase; margin-top:3px; letter-spacing:.3px; }}
  .pill.blue {{ background:#eff6ff; }}
  .pill.blue .pill-val {{ color:#1d4ed8; }}
  .pill.green {{ background:#f0fdf4; }}
  .pill.green .pill-val {{ color:#15803d; }}

  /* ── PILL GRIDS (responsive for player modal) ── */
  .pills-key {{ display:grid; grid-template-columns:repeat(2,1fr); gap:7px; margin-bottom:10px; }}
  .pills-counts {{ display:grid; grid-template-columns:repeat(3,1fr); gap:6px; margin-bottom:12px; }}
  @media (min-width:420px) {{
    .pills-key {{ grid-template-columns:repeat(4,1fr); }}
    .pills-counts {{ grid-template-columns:repeat(4,1fr); }}
  }}
  ...
  <div class="header-inner">
    <div class="header-top">
      <div>
        ...
        <div class="header-subtitle">2026 Season · Live Statistics</div>
      </div>
      <div class="header-date">
        <div class="header-date-label">Updated</div>
...
function renderPlayerCards(data) {{
  ...
  data.forEach(p => {{
    ...
    card.innerHTML=`
      <div style="background:${{c.bg}};padding:14px 8px 10px;text-align:center">
        <div style="width:46px;height:46px;border-radius:50%;background:${{c.accent}};margin:0 auto 7px;display:flex;align-items:center;justify-content:center;font-weight:900;color:${{c.bg}};font-size:17px">${{p.jersey||'?'}}</div>
        <div style="color:#fff;font-weight:800;font-size:13px;line-height:1.25;letter-spacing:-.1px">${{p.name}}</div>
        <div style="color:rgba(255,255,255,.6);font-size:12px;margin-top:3px">${{roles.join(' ')}}</div>
      </div>
      <div style="padding:9px 10px 8px;text-align:center;background:#fff;border-top:2px solid ${{c.accent}}">
        ${{p.hitting?`<div style="color:#1e3a5f;font-weight:800;font-size:14px">AVG <span style="font-size:15px">${{fmt(p.hitting.avg)}}</span></div>`:''}}
        ${{p.pitching?`<div style="color:#1a4731;font-weight:800;font-size:14px">ERA <span style="font-size:15px">${{p.pitching.era}}</span></div>`:''}}
        <div style="color:#94a3b8;font-size:10px;margin-top:3px;font-weight:600">${{p.team}}</div>
      </div>`;
...
function openPlayerModal(name, team) {{
  ...
  let body=`
    ...
    <div class="modal-hdr" style="background:${{c.bg}}">
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-top:4px">
        <div style="width:46px;height:46px;border-radius:50%;background:${{c.accent}};display:flex;align-items:center;justify-content:center;font-weight:900;color:${{c.bg}};font-size:18px;flex-shrink:0">${{player.jersey||'?'}}</div>
        <div>
          <div style="font-size:19px;font-weight:800;line-height:1.1">${{player.name}}</div>
          <div style="opacity:.75;font-size:12px;margin-top:2px">${{team}}</div>
        </div>
      </div>
    </div>
    <div class="modal-body">`;
  if (h) {{
    body+=`
      <div style="font-weight:700;font-size:13px;color:#1e3a5f;margin-bottom:8px">🥎 Hitting</div>
      <div class="pills-key">
  ...
  if (pi) {{
    const sep=h?'padding-top:12px;border-top:1px solid #f1f5f9;margin-top:4px;':'';
    body+=`
      <div style="font-weight:700;font-size:13px;color:#1a4731;margin-bottom:8px;${{sep}}">⚾ Pitching</div>
      <div class="pills-key">
