...
<style>
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  /* ── STAT PILLS ── */
  .pill {{ background:#f8fafc; border-radius:8px; padding:6px 3px; text-align:center; }}
  .pill-val {{ font-size:13px; font-weight:800; color:#0f2744; line-height:1; }}
  .pill-lbl {{ font-size:8px; color:#94a3b8; font-weight:600; text-transform:uppercase; margin-top:2px; letter-spacing:.2px; }}
  .pill.blue {{ background:#eff6ff; }}
  .pill.blue .pill-val {{ color:#1d4ed8; font-size:14px; }}
  .pill.green {{ background:#f0fdf4; }}
  .pill.green .pill-val {{ color:#15803d; font-size:14px; }}

  /* ── PILL GRIDS — always 4-col, no horizontal overflow ── */
  .pills-key {{ display:grid; grid-template-columns:repeat(4,1fr); gap:5px; margin-bottom:7px; }}
  .pills-counts {{ display:grid; grid-template-columns:repeat(4,1fr); gap:5px; margin-bottom:10px; }}
